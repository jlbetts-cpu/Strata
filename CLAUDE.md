...
- **The mark is `StrataMarkShape`, not a letter in a font** (2026-09-09).
  ...
  and do not ship a bitmap of it. The slash is 0.56 of the width: rendered at
  0.36/0.43/0.50/0.56/0.62 side by side, below 0.43 it reads as two bars
  joined by a hairline and past 0.56 the counters close into a lozenge.
  **The interior angles round too**, at about half the outer radius. They were
  knife-sharp on the rule that a block has no rounded inside corners — true of
  a block, and not of a letter cut from one, where the two acute corners came
  to points sharp enough to read as a rendering artefact.
