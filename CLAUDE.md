...
- **The mark is a BLOCK with the letter on it**, not a bare letter.
  ...
  white over it, and the app icon is the same idea at tile scale — the S
  itself is the block, in WHITE, on the app's warm black, with the rim along
  its top edge and the frosted band across its bottom 26%. A flat letter has
  neither and reads as a sticker. Four combinations were rendered and compared
  at 1024 and at 60pt: a pink field leaves the letter nothing to catch,
  pink-on-pink is prettier large and turns to mush small, and white on black
  is both the owner's call and the highest contrast of the four — which is
  what an icon is judged on at 60pt.
