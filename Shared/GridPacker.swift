...
/// now has several callers — the day tower, the month tower, the onboarding
/// tower and the home screen widget — and it is the one function that must
/// stay identical between them. A month whose blocks settle by different rules
/// than the tower's stops looking like the same object, and so does a widget:
/// "make sure the tower works the same way it does in the app."
///
/// **In Shared/ so the widget compiles the same code, not a copy of it.** The
/// widget had its own first-fit for one build, which is the arrangement
/// drifting the moment either is touched. The column count is a parameter
/// rather than a read of `GridConstants` for the same reason — that type
/// belongs to the app target, and a shared rule cannot depend on it.
enum GridPacker {
